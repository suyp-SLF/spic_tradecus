package spic.cus.scmc.salcontract.formplugin;
...
public class SalcontractEditPlugin extends AbstractBillPlugIn implements HyperLinkClickListener{

	@Override
	public void beforeBindData(EventObject e) {
		// TODO Auto-generated method stub
		super.beforeBindData(e);
		BillList spic_arlist = this.getControl("spic_arlist");
		Object billid = this.getModel().getDataEntity().getPkValue();
		Set<Long> ids = new HashSet<Long>();
		if(!"0".equals(billid.toString())) {
			
			Map<String, HashSet<Long>> targetBillsMap = BFTrackerServiceHelper.findTargetBills("conm_salcontract", new Long[] {(Long) billid});
			ids = targetBillsMap.get("ar_finarbill");
			
		}
		FilterParameter filterParameter = new FilterParameter();
		filterParameter.setFilter(new QFilter("id", QCP.in, ids));
		spic_arlist.setFilterParameter(filterParameter);
	}
	...
	public void registerListener(EventObject e) {
		...
		BillList billlist = this.getControl("spic_arlist");
